...
source("./Conjugate_SABRE/C_SABRE_gamma_density.R")

C_SABRE<-function(y,X,Z,iters,it_count,
  ...
  for(i in 2:iters){
    ...
    mean_w<-var_w%*%((t(Xg)%*%(y - Z%*%b[i-1,])) + Mu_w[g_ind]/sigsq_W[g_ind])
    ...
    if(NE_curr){
      ...
      for(p in 2:(length(sigsq_w_change)+1)){
        ...
        MEAN_mu<-VAR_mu*(sum(wf[i,gg1])/sigsq_w[i-1,p] + (mu_hyp[p]/var_mu_w[p]))
        mu_w[i,p]<-rnorm(1,MEAN_mu,sqrt(sigsq_e[i-1]*VAR_mu))
        ALPHA_sigsq_w<-((sum(gamma[i-1,change_start[p]:change_end[p]]) + 2*alpha_w[p])/2)
        BETA_sigsq_w<-(beta_w[p] + 0.5*sum((wf[i,gg1]-mu_w[i,p])^2)/sigsq_e[i-1])
    ...
    if(NE_curr){
      a_e<-0.5*(length(y) + length(sigsq_w_change) + sum(gamma[(i-1),])) + alpha_e
      b_e<-beta_e + 0.5*(sum((y-Xg%*%wf[i,g_ind] - Z%*%b[i,])^2) + t((wf[i,g_ind]-Mu_w[g_ind]))%*%diag((1/sigsq_W[g_ind]),length(sigsq_W[g_ind]))%*%(wf[i,g_ind]-Mu_w[g_ind]) + t(mu_w[i,-1]-mu_hyp[-1])%*%diag((1/var_mu_w),length(var_mu_w[-1]))%*%(mu_w[i,-1]-mu_hyp[-1]))
      sigsq_e[i]<-rigamma(1,a_e,b_e)
    }
    else{
      a_e<-0.5*(length(y) + sum(gamma[(i-1),])) + alpha_e
      b_e<-beta_e + 0.5*(sum((y-Xg%*%wf[i,g_ind] - Z%*%b[i,])^2) + t((wf[i,g_ind]-Mu_w[g_ind]))%*%diag((1/sigsq_W[g_ind]),length(sigsq_W[g_ind]))%*%(wf[i,g_ind]-Mu_w[g_ind]))
      sigsq_e[i]<-rigamma(1,a_e,b_e)
    }
    ...
    if(method=="fixed"){
      ...
      if(NE_prop==TRUE & NE_curr==TRUE){
        ...
        post_prop<-SSdensity.1(NY,X,XTX,gamma_prop,sigsq_e[i],YZB,Mu_w,sigsq_W,pi_a,pi_b,pi[i])$logmarg
        q_prop<-sum(dbern(prop,PIP1,log=TRUE))
        post_curr<-SSdensity.1(NY,X,XTX,gamma_curr,sigsq_e[i],YZB,Mu_w,sigsq_W,pi_a,pi_b,pi[i])$logmarg
        ...
        q_curr<-sum(dbern(curr,PIP2,log=TRUE))
        post_prop<-SSdensity.1(NY,X,XTX,gamma_prop,sigsq_e[i],YZB,Mu_W_prop,sigsq_W_prop,pi_a,pi_b,pi[i])$logmarg
        ...
        post_prop<-SSdensity.1(NY,X,XTX,gamma_prop,sigsq_e[i],YZB,Mu_w[1],sigsq_W[1],pi_a,pi_b,pi[i])$logmarg
      ...
      gind<-(1:NX)*gamma[i,]
      g_ind<-gind[gind!=0]
      m<-SSdensity.1(NY,X,XTX,gamma[i,],sigsq_e[i],YZB,Mu_w,sigsq_W,pi_a,pi_b,pi[i])
      XTXg<-m$XTXg
      Xg<-m$Xg
    }
